from azure.mgmt.logic import LogicManagementClient
from azure.identity import InteractiveBrowserCredential
from azure.core.exceptions import ResourceNotFoundError
import copy

# Configurações
subscription_id = 'd5005b6c-0f96-460d-bbe3-39dc6fe24109'
resource_group = 'centric-prd-rg'
#listagem de LA que deseja inserir novos IPS
listLogicAppName = ['Centric.Sales.Apttus.ExpeditionParkUpdated','Centric.Sales.Apttus.GalpCompanyDeleted','Centric.Sales.Apttus.GalpCompanyUpdated','Centric.Sales.Apttus.GalpWarrantyCreated','Centric.Sales.Apttus.GalpWarrantyDeleted','Centric.Sales.Apttus.GalpWarrantyUpdated','Centric.Sales.Apttus.LubricantsAccountCreate','Centric.Sales.Apttus.LubricantsAccountDelete','Centric.Sales.Apttus.QuoteInsert','Centric.Sales.Apttus.QuoteUpdate','Centric.Sales.galp_agreement.ContratoUpsert_PRD','Centric.Sales.GetArchivedSEPAMandate','Centric.Sales.GetMeterRead','Centric.Sales.GetOpportunityStatus','Centric.Sales.PublishMeterRead','CEntriC.Sales.Sharepoint_Agreement','Centric.Sales.SubmitAgreementToEcommerce','Centric.Sales.upsertSEPAMandate','Centric.Sales.upsertSEPAMandateERP','Centric.Sales.WonQuote'] 
# Listagem de IP que quer permitir
listIpCsv = ["4.175.0.0/16","4.180.0.0/16","4.210.128.0/17","4.231.0.0/17","4.245.0.0/17","9.163.0.0/16","13.69.0.0/17","13.73.128.0/18","13.73.224.0/21","13.80.0.0/15","13.88.200.0/21","13.93.0.0/17","13.94.128.0/17","13.95.0.0/16","13.104.145.192/26","13.104.146.0/26","13.104.146.128/25","13.104.158.176/28","13.104.209.0/24","13.104.214.0/25","13.104.218.128/25","13.105.22.0/24","13.105.23.128/25","13.105.28.32/28","13.105.29.128/25","13.105.60.48/28","13.105.60.96/27","13.105.60.128/27","13.105.66.144/28","13.105.105.96/27","13.105.105.128/28","13.105.105.160/27","20.4.0.0/16","20.8.0.0/16","20.13.0.0/17","20.16.0.0/16","20.23.0.0/16","20.31.0.0/16","20.33.1.0/24","20.33.2.0/24","20.33.6.0/24","20.33.21.0/24","20.33.25.0/24","20.33.33.0/24","20.33.36.0/24","20.33.39.0/24","20.33.49.0/24","20.33.66.0/24","20.33.72.0/24","20.33.86.0/24","20.33.92.0/24","20.33.110.0/24","20.33.145.0/24","20.33.163.0/24","20.33.198.0/24","20.33.207.0/24","20.33.217.0/24","20.33.232.0/24","20.33.244.0/24","20.38.108.0/23","20.38.200.0/22","20.47.7.0/24","20.47.18.0/23","20.47.30.0/24","20.47.96.0/23","20.47.115.0/24","20.47.118.0/24","20.50.0.0/18","20.50.88.0/21","20.50.128.0/17","20.54.128.0/17","20.56.0.0/16","20.60.26.0/23","20.60.130.0/24","20.60.150.0/23","20.60.196.0/23","20.60.222.0/23","20.60.250.0/23","20.61.0.0/16","20.67.0.0/17","20.71.0.0/16","20.73.0.0/16","20.76.0.0/16","20.82.0.0/17","20.86.0.0/16","20.93.128.0/17","20.95.64.0/24","20.95.66.0/24","20.95.68.0/23","20.95.72.0/23","20.95.76.0/23","20.95.81.0/24","20.95.98.0/24","20.95.103.0/24","20.95.107.0/24","20.101.0.0/16","20.103.0.0/16","20.105.128.0/17","20.107.0.0/17","20.123.128.0/17","20.126.0.0/16","20.135.24.0/23","20.135.140.0/22","20.135.144.0/23","20.143.1.0/24","20.143.32.0/23","20.143.39.0/24","20.143.50.0/23","20.143.80.0/22","20.150.8.0/23","20.150.37.0/24","20.150.42.0/24","20.150.74.0/24","20.150.76.0/24","20.150.83.0/24","20.150.122.0/24","20.152.46.0/23","20.153.4.0/24","20.153.31.0/24","20.153.130.0/24","20.153.141.0/24","20.153.150.0/23","20.153.152.0/24","20.153.154.0/24","20.157.18.0/24","20.157.21.0/24","20.157.22.0/24","20.157.30.0/24","20.157.33.0/24","20.157.97.0/24","20.157.115.0/24","20.157.123.0/24","20.157.146.0/24","20.157.158.0/24","20.157.170.0/24","20.157.186.0/24","20.157.212.0/24","20.157.221.0/24","20.157.223.0/24","20.157.230.0/24","20.157.245.0/24","20.160.0.0/16","20.190.137.0/24","20.190.160.0/24","20.201.216.0/24","20.202.2.0/24","20.202.12.0/22","20.202.16.0/22","20.202.140.0/24","20.202.150.0/24","20.202.152.0/24","20.202.154.0/24","20.202.156.0/24","20.202.158.0/24","20.202.160.0/24","20.202.162.0/24","20.202.164.0/24","20.202.166.0/24","20.202.168.0/24","20.202.236.0/24","20.202.248.0/24","20.202.250.0/23","20.209.10.0/23","20.209.48.0/23","20.209.72.0/23","20.209.76.0/23","20.209.108.0/23","20.209.192.0/23","20.209.230.0/23","20.224.0.0/16","20.229.0.0/16","20.234.128.0/17","20.238.128.0/17","23.97.128.0/17","23.98.46.0/24","23.100.0.0/20","23.101.64.0/20","40.64.144.32/27","40.64.172.128/25","40.67.192.0/19","40.68.0.0/16","40.74.0.0/18","40.78.210.0/24","40.79.205.192/27","40.79.205.224/28","40.79.206.0/27","40.82.92.0/22","40.87.184.0/22","40.90.17.64/27","40.90.18.192/26","40.90.20.128/25","40.90.21.0/25","40.90.130.0/27","40.90.133.0/27","40.90.134.64/26","40.90.134.128/26","40.90.138.0/27","40.90.141.32/27","40.90.141.160/27","40.90.142.224/28","40.90.144.192/27","40.90.145.192/27","40.90.146.16/28","40.90.146.128/27","40.90.150.128/25","40.90.157.64/26","40.90.159.0/24","40.91.28.0/22","40.91.192.0/18","40.93.65.0/24","40.101.2.192/26","40.101.3.0/25","40.101.20.64/26","40.101.20.128/25","40.112.36.128/25","40.112.37.0/26","40.112.38.192/26","40.112.96.0/19","40.113.96.0/19","40.113.128.0/18","40.114.128.0/17","40.115.0.0/18","40.118.0.0/17","40.119.128.0/19","40.120.152.0/22","40.123.140.0/22","40.123.180.0/22","40.126.9.0/24","40.126.32.0/24","48.208.72.0/24","48.208.216.0/24","48.209.0.0/17","48.209.192.0/18","48.213.128.0/25","48.213.128.128/26","48.222.128.0/17","50.85.0.0/16","51.5.11.0/24","51.5.12.0/24","51.5.255.208/28","51.5.255.224/28","51.105.96.0/19","51.105.128.0/17","51.124.0.0/16","51.136.0.0/16","51.137.0.0/17","51.137.192.0/18","51.138.0.0/17","51.144.0.0/16","51.145.128.0/17","52.101.69.0/24","52.101.70.0/23","52.101.72.0/23","52.102.161.0/24","52.103.33.0/24","52.103.161.0/24","52.106.139.0/24","52.106.184.128/27","52.108.24.0/21","52.108.56.0/21","52.108.80.0/24","52.108.108.0/23","52.108.110.0/24","52.109.88.0/22","52.111.243.0/24","52.112.14.0/23","52.112.17.0/24","52.112.18.0/23","52.112.22.0/24","52.112.83.0/24","52.112.97.0/24","52.112.98.0/23","52.112.102.0/24","52.112.110.0/24","52.112.124.0/24","52.112.144.0/20","52.112.197.0/24","52.112.216.0/21","52.112.228.128/25","52.112.233.0/24","52.112.237.0/24","52.112.238.0/24","52.113.9.0/24","52.113.12.0/24","52.113.37.0/24","52.113.38.0/23","52.113.69.0/24","52.113.83.0/24","52.113.85.0/24","52.113.86.0/24","52.113.130.0/24","52.113.144.0/21","52.113.199.0/24","52.114.72.0/22","52.114.241.0/24","52.114.242.0/24","52.114.252.0/22","52.115.226.0/23","52.115.228.0/23","52.120.128.0/21","52.120.208.0/20","52.121.24.0/21","52.121.64.0/20","52.122.20.0/22","52.122.96.0/20","52.122.112.0/21","52.123.7.0/24","52.123.18.0/24","52.123.133.0/24","52.123.134.0/23","52.123.195.0/24","52.125.140.0/23","52.136.192.0/18","52.137.0.0/18","52.142.192.0/18","52.143.0.0/18","52.143.194.0/24","52.143.208.0/24","52.148.192.0/18","52.149.64.0/18","52.157.64.0/18","52.157.128.0/17","52.166.0.0/16","52.174.0.0/16","52.178.0.0/17","52.232.0.0/17","52.232.147.0/24","52.233.128.0/17","52.236.128.0/17","52.239.140.0/22","52.239.212.0/23","52.239.242.0/23","52.245.48.0/22","52.245.124.0/22","57.150.2.0/23","57.150.80.0/23","57.150.90.0/23","57.150.110.0/23","57.150.150.0/23","57.150.166.0/23","57.150.224.0/23","57.153.0.0/16","57.157.12.0/24","57.157.13.0/29","65.52.128.0/19","68.219.160.0/19","70.152.36.0/24","70.152.181.0/24","70.152.182.0/23","70.152.184.0/21","70.152.192.0/20","70.152.208.0/23","70.152.210.0/24","72.145.128.0/18","74.178.128.0/17","74.234.128.0/17","98.64.0.0/16","98.71.128.0/17","104.40.128.0/17","104.44.89.160/27","104.44.90.192/27","104.44.93.0/27","104.44.93.192/27","104.44.95.80/28","104.44.95.96/28","104.45.0.0/18","104.45.64.0/20","104.46.32.0/19","104.47.128.0/18","104.47.216.64/26","104.214.192.0/18","108.141.0.0/16","108.142.0.0/15","128.251.128.0/17","132.164.0.0/17","132.220.0.0/16","134.149.128.0/17","135.236.0.0/17","137.116.192.0/19","137.117.128.0/17","151.206.71.0/24","151.206.72.0/24","151.206.92.0/23","151.206.139.0/24","157.55.8.64/26","157.55.8.144/28","168.61.56.0/21","168.63.0.0/19","168.63.96.0/19","172.201.0.0/16","172.205.128.0/17","172.211.0.0/16","191.233.64.0/18","191.237.232.0/22","191.239.200.0/22","193.149.80.0/21","213.199.128.0/20","213.199.180.32/28","213.199.180.96/27","213.199.180.192/27","213.199.183.0/24","2603:1020:200::/46","2603:1020:205::/48","2603:1020:206::/47","2603:1020:208::/56","2603:1026:900:7::/64","2603:1026:900:8::/63","2603:1026:900:1a::/63","2603:1026:900:1c::/64","2603:1026:2405::/48","2603:1026:2500:24::/64","2603:1026:3000:140::/59","2603:1027:1:140::/59","2603:1061:1311:5800::/54","2603:1061:1601::/63","2603:1061:1601:2::/64","2603:1061:171d::/48","2603:1061:2000:680::/62","2603:1061:2002:800::/56","2603:1061:2004:7800::/56","2603:1061:2010:19::/64","2603:1061:2011:19::/64","2603:1062:c:14::/63","2603:1063:21::/56","2603:1063:ff::/64","2603:1063:121::/55","2603:1063:121:200::/56","2603:1063:221::/55","2603:1063:420::/56","2603:1063:62b::/56","2603:1063:1c12::/55","2603:1063:2206:24::/64","2a01:111:f403:c201::/64","2a01:111:f403:ca05::/64","2a01:111:f403:ca06::/63","2a01:111:f403:ca08::/63","2a01:111:f403:d201::/64","2a01:111:f403:da01::/64","2a01:111:f403:e201::/64"]  
currentList = []

# Replace this with your required scope
scope = "https://management.azure.com/.default"

# Authenticate interactively via browser
credential = InteractiveBrowserCredential()

# Autenticação
client = LogicManagementClient(credential, subscription_id)

try:
    for logic_app_name in listLogicAppName:
        # Pega a definição atual da Logic App
        logic_app = client.workflows.get(resource_group, logic_app_name)
        # Validando se a LA possui restricao de ip habilitada
        if getattr(logic_app, 'access_control', None) is not None:
            definition = logic_app.access_control

            # Copia a definição para alterar
            new_definition = copy.deepcopy(definition)

            trigger = new_definition.triggers
            if getattr(trigger, 'allowed_caller_ip_addresses', None) is not None:
                allowed_ips = trigger.allowed_caller_ip_addresses
                for ip in allowed_ips:
                    # Adiciona o IP se ainda não estiver na lista
                    if ip.address_range:
                        currentList.append(ip.address_range)
                
                finalList = [item for item in listIpCsv if item not in currentList]
                for ip in finalList:
                    newIp = {
                        "additional_properties": {},
                        "address_range": ip
                    }
                    allowed_ips.append(newIp)

                if finalList:
                    # Atualiza a definição da Logic App
                    workflow_params = {
                        'location': logic_app.location,
                        'access_control': new_definition,
                        'definition': logic_app.definition,
                        'tags': logic_app.tags
                    }

                    if logic_app.parameters:
                        workflow_params = {
                            'location': logic_app.location,
                            'access_control': new_definition,
                            'definition': logic_app.definition,
                            'parameters': logic_app.parameters,
                            'tags': logic_app.tags
                        }

                    # Faz a atualização (replace)
                    client.workflows.create_or_update(resource_group, logic_app_name, workflow_params)

                    print(f"IP {finalList} adicionado em Logic App {logic_app_name}")
                else:
                    print(f"Nenhum IP foi adicionado. Todos os IPs da listagem já existem Logic App {logic_app_name}")
            else:
                print(f"A Logic App {logic_app_name} nao possui restricoes de IP")
        else:
            print(f"Logic App {logic_app_name} nao possui restricao de ip")
except ResourceNotFoundError:
    print(f"A Logic App {logic_app_name} nao existe")